...
class Tracker:
    def __init__(self, _cap, _video_out, list_params):
        ...
        self.show_3Dpoints_holistic = list_params['show_3Dpoints_holistic']
        self.show3D_one_points = list_params['show3D_one_points']
        self.show_BBOX = list_params['show_BBOX']
    ...
    def InitTracker(self):
        ...
        if self.TrackObjects is not None:
            ...
            print(w, h)
            #self.out = cv2.VideoWriter(self.video_out, cv2.VideoWriter_fourcc(*'XVID'), 30.0, (w, h)) СОХРАНЕНИЕ ВИДЕО
            frame_counter = 0
    ...
    def TrackImage(self):
        ...
        if self.predictor_type == 4:  # facenet_pytorch
        ...
        #Не очень понял, что вообще рисует. Если комитить ничего не меняется. Рисуем Ббокс лаймового цвета. Время почти не занимает
        if len(self.pts['coords']) > 0:
        ...
        ######  рисуем id, ббоксы. Занимает ОЧЕНЬ много времени
        if self.show_BBOX:
        ...
        if self.show_BBOX:
            ### сохраняем результат. 0.02 секунды кадра занимает
            ...
            #self.out.write(cv2.cvtColor(frame1, cv2.COLOR_BGR2RGB)) #Для сохранения видео

            if self.frame_counter > 0:
                
                ### вывод результата на экран. 0.04 секунды кадра занимает
                t_res = cv2.cvtColor(frame1, cv2.COLOR_BGR2RGB)
                cv2.imshow("preview", cv2.resize(t_res, (1600, 1000)))
        ...
        if self.show_3Dpoints_holistic:
            self.TrackObjects.transate_landmarks_to_3d(frame1,self.compensaition_index,self.collect_confiedense)

        #Если сохранять фотографию то жрет кучу времени. Без сохранения быстро
        if self.show3D_one_points:
            self.TrackObjects.transate_one_landmark_to_3d(frame1)  
